...
1. **Algorithm Choice**: "I used Token Bucket because it allows bursts while maintaining average rate."

2. **Distributed Systems**: "For multi-server deployments, Redis provides shared state using atomic Lua scripts."

3. **Interface Design**: "The `Limiter` interface allows easy swapping between implementations without changing business logic."

4. **Production Considerations**:
   - "Fail open on Redis errors to avoid blocking legitimate traffic"
   - "Automatic cleanup prevents memory leaks"
   - "Per-IP limiting protects against individual abusers"

5. **Scalability**: "Redis-based limiter scales horizontally with your application servers."
